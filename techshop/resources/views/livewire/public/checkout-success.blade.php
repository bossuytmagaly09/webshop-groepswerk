<section class="pt-16 pb-20 max-w-[700px] mx-auto px-6 text-center">
    ...
    @else
        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400 mb-6 border border-red-100 dark:border-red-500/20">
            <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <circle cx="12" cy="12" r="10"/><line x1="15" x2="9" y1="9" y2="15"/><line x1="9" x2="15" y1="9" y2="15"/>
            </svg>
        </div>

        <div class="text-[11px] font-mono text-red-600 dark:text-red-400 tracking-widest uppercase mb-3">
            {{ __('Payment Failed') }}
        </div>
        ...
        <div class="mb-10">
            ...
            <p class="text-[#666666] dark:text-zinc-400 text-[16px] mb-8 max-w-md mx-auto">
                {{ __('Your payment could not be processed successfully. The order has been cancelled, but you can try again or contact support.') }}
            </p>
            
            <div class="flex flex-col items-center gap-4 bg-red-50/50 dark:bg-red-500/5 rounded-2xl p-6 border border-red-100/50 dark:border-red-500/10 mb-8">
                <p class="text-sm text-red-700 dark:text-red-400 font-medium">
                    {{ __('Test card failure simulated or actual error occurred.') }}
                </p>
                <a 
                    href="{{ route('cart.index') }}" 
                    wire:navigate
                    class="bg-[#0d0d0d] dark:bg-white text-white dark:text-[#0d0d0d] px-6 py-2 rounded-full text-[14px] font-bold hover:opacity-90 transition-all flex items-center gap-2"
                >
                    {{ __('Back to Cart') }}
                </a>
            </div>
    ...
    <div class="bg-[#fafafa] dark:bg-zinc-900 border border-black/[0.05] dark:border-white/[0.05] rounded-[24px] p-6 mb-8 text-left">
        ...
        <div class="border-t border-black/[0.06] dark:border-white/[0.05] pt-4 space-y-3">
            ...
            @if($order->stripe_payment_intent_id)
                <div class="border-t border-black/[0.06] dark:border-white/[0.05] pt-3 flex justify-between text-[12px] font-mono text-[#999999]">
                    <span>{{ __('Transaction ID') }}</span>
                    <span class="break-all ml-4 text-right">{{ $order->stripe_payment_intent_id }}</span>
                </div>
            @endif
        </div>
    </div>
