    ...
    <div class="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div class="flex items-center gap-1">
            ...
            <div class="relative">
                @auth
                    <button
                        ...
                        class="inline-flex items-center justify-center w-9 h-9 rounded-full hover:bg-gray-100 transition-colors"
                    ...
                    <div
                        ...
                        class="absolute right-0 mt-2 w-44 bg-white rounded-xl border border-black/[0.07] shadow-lg py-1 z-50"
                    >
                        <a
                            href="{{ route('dashboard') }}"
                            wire:navigate
                            @click="accountOpen = false"
                            class="flex items-center gap-2 px-4 py-2.5 text-[14px] hover:bg-gray-50 transition-colors"
                        >
                            {{ __('Dashboard') }}
                        </a>
                        <div class="my-1 border-t border-black/[0.05]"></div>
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit" class="w-full text-left flex items-center gap-2 px-4 py-2.5 text-[14px] hover:bg-gray-50 transition-colors text-red-500">
                                {{ __('Log out') }}
                            </button>
                        </form>
                    </div>
            ...
            <button
                type="button"
                @click="open = !open"
                class="md:hidden inline-flex items-center justify-center w-9 h-9 rounded-full border border-black/[0.08] hover:bg-gray-50 transition-colors ml-1"
                :aria-expanded="open"
